...
public class InstanceStreamClassifier {
	...
	protected void trainClassifier(Instances stream) throws Exception {
		...
		for (int i = 0; i < stream.numInstances(); i++) {
			...
			grantedProb= hTree.distributionForInstance(inst)[indexOfGranted];
			deniedProb= hTree.distributionForInstance(inst)[indexOfDenied];
			...
			if (numOfInstances%5000 == 0) {
				numOfNodesArr[curIndexOfNumOfNodes]= hTree.getNumOfNodes();
