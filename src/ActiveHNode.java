...
public class ActiveHNode extends LeafNode implements LearningNode, Serializable {
  ...
  int saIndex= 8;
  public void setSaIndex(int saIndex) {
  ...
  // get m_nodeStats
  public Map<String, ConditionalSufficientStats> get_m_nodeStats(){
	  return m_nodeStats;
  }
      ...
      if (i != inst.classIndex()) {
        ...
        stats
            .update(inst.value(a),
                ...
                inst.weight(), (int) inst.value(saIndex));
        /**added sensitive attribute value to update discrimination related stats*/
      }
    }
  }
  ...
  public List<SplitCandidate> getPossibleSplits(SplitMetric splitMetric, ActiveHNode node) {
     ...
     * check null split
     * */
  //SplitCandidate(the splitting test, postSplitDists, the merit of the split)
    ...
    SplitCandidate nullSplit = new SplitCandidate(null, nullDist,
        splitMetric.evaluateSplit(m_classDistribution, nullDist, null, -1, node));
    				/** null: no splitting attribute name, -1: discrimination stays unchange*/
    ...
    for (Map.Entry<String, ConditionalSufficientStats> e : m_nodeStats
      ...
      /* bestSplit among all possible splits of numeric attribute and only one for nominal*/
      SplitCandidate splitCandidate = stat.bestSplit(splitMetric,
          m_classDistribution, e.getKey() /*i.e. attribute name*/, node);
